...
exports.getAllProducts = async (req, res) => {
  try {
    ...
    const { sortBy } = req.query; // 'lowToHigh' for low to high, 'highToLow' for high to low
  
    // Build the sort object based on the provided sortBy value
    let sortOrder = {};
    
    // Handle different sort options
    if (sortBy === 'lowToHigh') { // Low to High
      sortOrder = { price: 1 }; // 1 for ascending order
    } else if (sortBy === 'highToLow') { // High to Low
      sortOrder = { price: -1 }; // -1 for descending order
    } else {
      sortOrder = { price: 1 }; // Default to low to high if no valid sort option is provided
    }
